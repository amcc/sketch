...
import { startFaceInput, reactToFace } from "./components/faceInput";
...
for (let x = 0; x < xGridSize; x++) {
  ...
  for (let y = 0; y < yGridSize; y++) {
    ...
    for (let z = 0; z < zGridSize; z++) {
      ...
      const instLines = new THREE.LineSegments(boxEdges, instMat);
...
function animate() {
  ...
  if (renderer.info.render.frame % 3 === 0) reactToFace(camRotation);
  //rotate parent
